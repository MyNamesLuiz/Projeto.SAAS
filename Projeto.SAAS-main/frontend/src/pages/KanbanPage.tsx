...
export default function KanbanPage() {
  ...
  const patchStatus = useMutation({
    ...
    onSuccess: () => {
      // Invalida OS e dashboard juntos — garante que métricas (receita,
      // contadores) reflitam a mudança imediatamente, sem esperar o interval
  ...
  const sensors = useSensors(
    useSensor(PointerSensor, { activationConstraint: isMobile ? { delay: 250, tolerance: 5 } : { distance: 4 } }),
  ...
  // Fix 1 — Collision detection customizada para multi-coluna:
  // pointerWithin tem prioridade (detecta a coluna onde o cursor está),
  // com fallback para rectIntersection quando o ponteiro está entre elementos.
  // Isso evita o "salto" de cards que closestCenter causava.
  const collisionDetection: CollisionDetection = useCallback((args) => {
    // Se há colisão direta com o ponteiro, usa ela (precisão máxima)
    const pointerCollisions = pointerWithin(args)
    if (pointerCollisions.length > 0) return pointerCollisions
    // Fallback: área de interseção de retângulos (evita drops no vácuo)
    return rectIntersection(args)
  }, [])

  // Fix 2 — Ref para evitar re-render de todas as colunas a cada pixel arrastado.
  // setOverId só é chamado quando a coluna de destino realmente muda.
  const prevOverId = useRef<OSStatus | null>(null)
  ...
  const onDragOver = useCallback((e: DragOverEvent) => {
    ...
    const isCol  = KANBAN_COLUMNS.some((c) => c.id === over.id)
    const newCol: OSStatus | null = isCol
      ? (over.id as OSStatus)
      : (displayList.find((o) => o.id === over.id)?._col ?? null)

    if (!newCol || newCol === prevOverId.current) return

    // Move o card visualmente para a nova coluna DURANTE o drag.
    // Sem isso o card fica preso na coluna original — não parece kanban.
    const draggedOS = displayList.find((o) => o.id === active.id)
    if (draggedOS && draggedOS._col !== newCol) {
      moveOS(draggedOS.id as number, newCol)
    }

    prevOverId.current = newCol
    setOverId(newCol)
  }, [displayList, moveOS])
  ...
  const onDragEnd = useCallback(async (e: DragEndEvent) => {
    ...
    // Usa o status real do servidor como "origem" — positions[os.id] já foi movido
    // otimisticamente no onDragOver, então usá-lo aqui causaria current === target sempre,
    ...
    // Posição já foi movida otimisticamente no onDragOver.
    // Aqui só persiste no servidor e reverte se falhar.
  ...
  const onDragCancel = useCallback(() => {
    // Reverte a posição otimista para o status original do servidor
    if (activeOS) {
      resetPosition(activeOS.id, toKanban(activeOS.status))
    }
    setActiveOS(null); setOverId(null); setShowHint(false)
    prevOverId.current = null
  }, [activeOS, resetPosition])

  if (isLoading) return (
    <div role="status" aria-live="polite" className="flex items-center justify-center h-full">
      <span className="font-mono text-[11px] tracking-widest text-[var(--text-muted)] uppercase animate-pulse">
        Carregando ordens...
      </span>
    </div>
  )
  ...
  if (isError) return (
    <div role="alert" className="flex items-center justify-center h-full">
      <span className="font-mono text-[11px] tracking-widest text-[var(--red)] uppercase">
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden -m-4 md:-m-5">
      ...
      {showHint && !isMobile && (
        <div role="status" aria-live="polite"
          className="absolute top-16 left-1/2 -translate-x-1/2 z-50 pointer-events-none
                     font-mono text-[10px] tracking-widest text-[var(--bg-base)]
                     bg-[var(--orange)] px-4 py-1.5 rounded-full">
      ...
      <div role="toolbar" aria-label="Ferramentas do Kanban"
        className="flex items-center gap-2 h-11 px-5 border-b border-[var(--border)] bg-[var(--bg-base)] flex-shrink-0">
        <div className="flex items-center gap-2 h-[28px] px-3 rounded-[3px] bg-[var(--bg-card)] border border-[var(--border)] flex-1 max-w-[340px]">
          <span className="text-[var(--text-muted)] text-[11px]" aria-hidden>⌕</span>
          <input
            value={search}
            onChange={(e) => setSearch(e.target.value)}
            aria-label="Buscar por placa, cliente ou modelo"
            placeholder="Buscar por placa, cliente ou modelo..."
            className="bg-transparent border-none outline-none font-mono text-[10px] text-[var(--text-primary)] placeholder-[var(--text-secondary)] w-full"
          />
          {search && (
            <button onClick={() => setSearch('')}
              className="font-mono text-[10px] text-[var(--text-secondary)] hover:text-[var(--text-primary)] bg-transparent border-none cursor-pointer">
              ✕
            </button>
          )}
        </div>
        ...
        <div className="ml-auto flex items-center gap-3">
          {patchStatus.isPending && (
            <span className="font-mono text-[9px] text-[var(--text-secondary)] tracking-widest">Salvando...</span>
          )}
          {search && (
          ...
          {totalParadas > 0 && (
            <span className="flex items-center gap-1.5 font-mono text-[9px] tracking-widest px-2.5 py-1 rounded-full"
              style={{ color: 'var(--red)', background: 'var(--red-dim)', border: '1px solid var(--red-border)' }}>
              <span aria-hidden className="w-1 h-1 rounded-full bg-[var(--red)] flex-shrink-0" />
      ...
      <DndContext sensors={sensors} collisionDetection={collisionDetection}
        ...
        // Fix 4 — autoScroll: rola o board automaticamente ao arrastar para as bordas
      ...
      {isMobile && (
        <div role="tablist" aria-label="Navegar entre colunas"
          className="flex items-center justify-center gap-1.5 py-2 flex-shrink-0 bg-[var(--bg-base)] border-t border-[var(--border)]">
          {KANBAN_COLUMNS.map((col, i) => {
            ...
            return (
              <button
                ...
                style={{ width: active ? 20 : 6, height: 6, background: active ? 'var(--orange)' : 'var(--border)' }}
              />
            )
          })}
        </div>
          ...
          >
            ...
            <div className="flex items-center gap-3 px-5 py-4"
              ...
              <div className="w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0"
                style={{ background: 'rgba(255,193,7,0.2)', border: '1px solid var(--amber-border)' }}>
                <svg width="16" height="16" viewBox="0 0 16 16" fill="none" aria-hidden>
                  <path d="M8 2L14.5 13H1.5L8 2Z" stroke="var(--amber)" strokeWidth="1.4" strokeLinejoin="round"/>
                  <line x1="8" y1="6.5" x2="8" y2="9.5" stroke="var(--amber)" strokeWidth="1.4" strokeLinecap="round"/>
                  <circle cx="8" cy="11.5" r="0.6" fill="var(--amber)"/>
                </svg>
              </div>
              <div>
                ...
                <p className="font-mono text-[9px] tracking-widest mt-0.5" style={{ color: 'var(--text-secondary)' }}>
                  OS-{String(blockModal.id).padStart(3, '0')} · {blockModal.veiculo_placa} · {blockModal.cliente_nome}
                </p>
              </div>
            </div>
            ...
            <div className="px-5 py-4 flex flex-col gap-4">
              <p className="font-mono text-[11px] leading-relaxed" style={{ color: 'var(--text-secondary)' }}>
                A OS está em{' '}
                <span style={{ color: 'var(--amber)', fontWeight: 700 }}>Ag. Aprovação</span>.
                {' '}Para avançar para as próximas etapas, é obrigatório informar o{' '}
                <span style={{ color: 'var(--text-primary)', fontWeight: 700 }}>Valor Final</span>{' '}
                do serviço — isso garante que a receita seja registrada corretamente.
              </p>

              {/* Destaque do campo que falta */}
              <div className="flex items-center justify-between px-3 py-2.5 rounded-[4px]"
  ...
  return (
    <div ref={setNodeRef} id={col.id} role="group"
      ...
      style={{
        ...
        background: isOver ? `${col.accent}14` : "var(--bg-card)",
        border: `1px solid ${isOver ? col.accent : "var(--border)"}`,
        borderStyle: isOver ? 'dashed' : 'solid',
      }}>
      <div className="flex items-center justify-between px-3 py-2.5" style={{ background: col.headerBg, borderBottom: `1px solid ${col.accent}40` }}>
        <span className="text-[10px] font-bold tracking-widest uppercase" style={{ fontFamily: "var(--font-heading)", color: col.color }}>{col.label}</span>
        <span className="font-mono text-[9px] font-bold px-1.5 py-0.5 rounded-full"
          style={{ background: `${col.accent}20`, color: col.color, border: `1px solid ${col.accent}30`, fontFamily: "var(--font-mono)", fontSize: 9, padding: "2px 6px", borderRadius: 4 }}>
          {cards.length}
        </span>
      </div>
        ...
        <div className="flex flex-col gap-1.5 p-2 min-h-[56px] flex-1">
          {cards.length === 0 ? (
            <div className="font-mono text-[9px] tracking-widest text-center py-5 transition-colors duration-150"
              style={{ color: isOver ? 'var(--orange)' : 'var(--border)' }}>
      ...
      <div className="px-2 py-1.5 border-t border-[var(--border)]">
        <button onClick={onAddClick}
          className="font-mono text-[9px] tracking-widest text-[var(--text-muted)] hover:text-[var(--text-secondary)]
...
function KanbanCard({
  ...
  return (
    <div ref={setNodeRef}
      ...
      style={{
        // Fix touchAction — obrigatório para o dnd-kit funcionar em produção.
        // Sem isso o browser intercepta os eventos de pointer para scroll
        // e o drag nunca é iniciado, especialmente em touch e em deploy.
        touchAction: 'none',
        transform: CSS.Transform.toString(transform),
        // Fix 3 — sem transition enquanto arrasta: evita o atraso/jank visual
        // dnd-kit já gerencia a animação de retorno; transition próprio causa conflito
        transition: isDragging ? 'none' : [transition, 'opacity 200ms ease', 'box-shadow 150ms ease'].filter(Boolean).join(', '),
        opacity: isDragging ? 0 : 1,  // invisível durante drag — DragOverlay é o visual
        cursor: isDragging ? 'grabbing' : 'grab',
        outline: 'none',
        // Fix GPU: força compositing layer no card ativo para eliminar repaint
        willChange: isDragging ? 'transform' : 'auto',
        background: isOverlay ? 'var(--bg-card)' : 'var(--bg-card)',
        border: `1px solid ${isOverlay ? 'var(--orange)' : os.prazo_vencido ? 'var(--red-border)' : os.alerta_parada ? 'var(--red-border)' : 'var(--border)'}`,
        borderRadius: 4, padding: '10px 11px',
        scale: isOverlay ? '1.02' : '1',
        boxShadow: isOverlay ? '0 8px 24px rgba(0,0,0,0.6), 0 0 0 1px var(--orange)' : 'none',
      }}
      onFocus={(e) => { e.currentTarget.style.boxShadow = '0 0 0 2px var(--orange)' }}
      onBlur={(e) => { e.currentTarget.style.boxShadow = isOverlay ? '0 8px 24px rgba(0,0,0,0.6), 0 0 0 1px var(--orange)' : 'none' }}
      onKeyDown={(e) => { if (e.key === 'Enter' && onClick) onClick() }}
      onClick={() => { if (!isDragging && onClick) onClick() }}
      {...attributes} {...listeners}>

      <div className="flex items-center justify-between mb-2">
        <span className="font-mono text-[8px] tracking-widest text-[var(--text-secondary)]">
          OS-{String(os.id).padStart(3, '0')}
        </span>
        <div className="flex items-center gap-1">
          {os.prazo_vencido && (
            <span className="font-mono text-[8px] tracking-widest px-1.5 py-0.5 rounded-[2px]"
              style={{ color: 'var(--red)', background: 'var(--red-dim)', border: '1px solid rgba(255,61,90,0.3)' }}>
              vencido
            </span>
          )}
          {os.alerta_parada && (
      ...
      <p className="font-mono text-[12px] font-bold text-[var(--text-primary)] mb-0.5 truncate">{os.cliente_nome}</p>
      <p className="font-mono text-[9px] text-[var(--text-secondary)] mb-2">{os.veiculo_modelo} {os.veiculo_ano}</p>
      <div className="h-px bg-[var(--border)] mb-2" />
      <p className="font-mono text-[8px] tracking-widest text-[var(--text-muted)] uppercase mb-1">Serviço</p>
      <p className="font-mono text-[10px] text-[var(--text-primary)] mb-2.5 truncate">{os.descricao_servico}</p>
      ...
      <div className="flex items-center justify-between">
        ...
        <span className="font-mono text-[11px] font-bold" style={{ color: valor ? 'var(--green)' : 'var(--text-secondary)' }}>
...
function OSDetailModal({ os, onClose }: { os: OS; onClose: () => void }) {
  const queryClient = useQueryClient()
  const [tab, setTab]       = useState<'info' | 'historico' | 'mover'>('info')
  ...
  const detailInputCls = "w-full bg-[var(--bg-base)] border border-[var(--border)] rounded-[3px] px-3 py-2 font-mono text-[11px] text-[var(--text-primary)] placeholder-[var(--text-secondary)] outline-none focus:border-[var(--orange)] transition-colors"
  const readCls        = "font-mono text-[11px] text-[var(--text-primary)]"
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4"
      ...
      <div className="bg-[var(--bg-base)] border border-[var(--border)] rounded-[6px] w-full max-w-xl max-h-[90vh] flex flex-col">

        {/* Header */}
        <div className="flex items-start justify-between px-5 py-4 border-b border-[var(--border)] flex-shrink-0">
          <div>
            <div className="flex items-center gap-2 mb-1">
              <span className="font-mono text-[9px] tracking-widest text-[var(--text-secondary)]">
                OS-{String(os.id).padStart(3, '0')}
              </span>
              {statusInfo && (
            ...
            <p className="font-mono text-[14px] font-bold text-[var(--text-primary)]">{os.cliente_nome}</p>
            <p className="font-mono text-[10px] text-[var(--text-secondary)]">{os.veiculo_placa} · {os.veiculo_modelo} {os.veiculo_ano}</p>
          </div>
          <div className="flex items-center gap-2">
            ...
            <button onClick={onClose}
              className="font-mono text-[14px] text-[var(--text-secondary)] hover:text-[var(--text-primary)] bg-transparent border-none cursor-pointer">
              ✕
            </button>
          </div>
        </div>
        ...
        <div className="flex border-b border-[var(--border)] flex-shrink-0">
          {(['info', 'historico', 'mover'] as const).map((t) => (
            <button key={t} onClick={() => setTab(t)}
              ...
              {t === 'info' ? 'Informações' : t === 'historico' ? 'Histórico' : '↕ Mover'}
            </button>
          ))}
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-5">
            ...
            <div className="flex flex-col gap-5">
              ...
              <InfoRow label="Prazo estimado">
                {editing
                  ...
                  : <span className={readCls} style={{ color: os.prazo_vencido ? 'var(--red)' : 'var(--text-primary)' }}>
              ...
              <div className="grid grid-cols-2 gap-4">
                <InfoRow label="Valor estimado">
                  {editing
                    ...
                    : <span className="font-mono text-[11px]" style={{ color: os.valor_estimado ? 'var(--green)' : 'var(--text-secondary)' }}>
                ...
                <InfoRow label="Valor final">
                  {editing
                    ...
                    : <span className="font-mono text-[11px] font-bold" style={{ color: os.valor_final ? 'var(--orange)' : 'var(--text-secondary)' }}>
              ...
              {saveError && (
                <p className="font-mono text-[10px] text-[var(--red)] tracking-widest">{saveError}</p>
              )}
              ...
              {editing && (
                <div className="flex gap-2 justify-end pt-2 border-t border-[var(--border)]">
                  ...
                  <button onClick={handleSave} disabled={update.isPending}
                    ...
                    style={{ background: 'var(--orange)', color: 'var(--bg-base)', border: 'none', opacity: update.isPending ? 0.6 : 1 }}>
              ...
              {loadingHist ? (
                <div className="flex flex-col gap-2">
                  {Array.from({ length: 4 }).map((_, i) => (
                    <div key={i} className="h-12 rounded-[3px] bg-[var(--bg-card)] animate-pulse" />
                  ))}
                </div>
              ) : historico.length === 0 ? (
                <div className="flex items-center justify-center py-10">
                  <span className="font-mono text-[10px] text-[var(--text-secondary)] tracking-widest">Nenhum histórico registrado</span>
                </div>
              ) : historico.map((h) => (
          ...
          {tab === 'mover' && (
            <div className="flex flex-col gap-3">
              <p className="font-mono text-[9px] tracking-widest text-[var(--text-secondary)] uppercase">Mover para qual etapa?</p>

              {/* Aviso inline se OS está em aprovação sem valor_final */}
              {toKanban(os.status) === 'aprovacao' && !os.valor_final && (
                <div className="flex items-start gap-3 px-4 py-3 rounded-[4px]"
                  style={{ background: 'var(--amber-dim)', border: '1px solid var(--amber-border)' }}>
                  <span style={{ fontSize: 14, flexShrink: 0, marginTop: 1 }}>⚠</span>
                  <div>
                    <p className="font-mono text-[10px] font-bold text-[var(--amber)] tracking-widest uppercase mb-1">
                      Valor Final obrigatório
                    </p>
                    <p className="font-mono text-[10px] text-[var(--text-secondary)] leading-relaxed">
                      Para avançar desta etapa, preencha o <strong style={{ color: 'var(--text-primary)' }}>Valor Final</strong> na aba Informações antes de mover.
                    </p>
                  </div>
                </div>
              )}
              ...
              {KANBAN_COLUMNS.map((col) => {
                ...
                return (
                  <button
                    key={col.id}
                    disabled={isCurrent || moveStatus.isPending || isBlocked}
                    onClick={() => moveStatus.mutate(STATUS_MAP_REVERSE[col.id])}
                    className="flex items-center gap-3 px-4 py-3 rounded-[4px] border cursor-pointer transition-all"
                    style={{
                      background: isCurrent ? `${col.accent}26` : isBlocked ? 'transparent' : 'var(--bg-card)',
                      border: `1px solid ${isCurrent ? col.color : isBlocked ? 'var(--border)' : 'var(--border)'}`,
                      opacity: moveStatus.isPending ? 0.6 : isBlocked ? 0.35 : 1,
                      cursor: isCurrent || isBlocked ? 'not-allowed' : 'pointer',
                    }}>
                    <span className="w-2 h-2 rounded-full flex-shrink-0" style={{ background: isBlocked ? 'var(--text-ghost)' : col.color }} />
                    <span className="font-mono text-[11px] font-bold" style={{ color: isBlocked ? 'var(--text-muted)' : 'var(--text-primary)' }}>{col.label}</span>
                    {isCurrent && (
                      <span className="ml-auto font-mono text-[8px] tracking-widest text-[var(--orange)]">ATUAL</span>
                    )}
                    {isBlocked && (
                      <span className="ml-auto font-mono text-[8px] tracking-widest" style={{ color: 'var(--amber)' }}>
                        requer valor final
                      </span>
                    )}
                  </button>
              ...
              {moveStatus.isError && (
                <p className="font-mono text-[10px] text-[var(--red)] tracking-widest">Erro ao mover. Tente novamente.</p>
              )}
            </div>
          )}
        </div>
...
function HistoricoItem({ h }: { h: HistoricoOS }) {
  ...
  return (
    <div className="flex items-center gap-3 px-3 py-2.5 rounded-[3px] bg-[var(--bg-card)] border border-[var(--border)]">
      <div className="flex items-center gap-2 flex-1 min-w-0">
        {anterior ? (
          <>
            <span className="font-mono text-[9px] text-[var(--text-secondary)] truncate">{anterior}</span>
            <span className="font-mono text-[9px] text-[var(--text-muted)] flex-shrink-0">→</span>
          </>
        ) : (
          <span className="font-mono text-[9px] text-[var(--text-secondary)] flex-shrink-0">criada em</span>
        )}
        <span className="font-mono text-[9px] font-bold text-[var(--orange)] truncate">{novo}</span>
      </div>
      <span className="font-mono text-[8px] text-[var(--text-muted)] flex-shrink-0">{data}</span>
    </div>
  )
}
...
function InfoRow({ label, value, children }: { label: string; value?: string; children?: React.ReactNode }) {
  return (
    <div>
      <p className="font-mono text-[8px] tracking-widest text-[var(--text-muted)] uppercase mb-1.5">{label}</p>
      {children ?? <span className="font-mono text-[11px] text-[var(--text-primary)]">{value ?? '—'}</span>}
    </div>
  )
}
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4"
      ...
      <div className="bg-[var(--bg-base)] border border-[var(--border)] rounded-[6px] w-full max-w-lg max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between px-5 py-4 border-b border-[var(--border)]">
          <div>
            <p className="font-mono text-[11px] tracking-widest text-[var(--text-primary)] uppercase">Nova Ordem de Serviço</p>
            <p className="font-mono text-[9px] text-[var(--text-secondary)] mt-0.5">Coluna: {colName}</p>
          </div>
        ...
        <div className="p-5 flex flex-col gap-4">
          ...
          <div className="flex gap-2 justify-end pt-2 border-t border-[var(--border)]">
            ...
            <button onClick={() => criar.mutate()} disabled={!canSubmit || criar.isPending}
              ...
              style={{ background: canSubmit ? 'var(--orange)' : 'var(--bg-card)', color: canSubmit ? 'var(--bg-base)' : 'var(--text-secondary)', border: 'none', opacity: criar.isPending ? 0.6 : 1 }}>
